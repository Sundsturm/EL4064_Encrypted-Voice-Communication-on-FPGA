...
architecture Behavioral of powercalcv1 is
    ...
    signal tempsin697, tempsin941, tempsin1477 : SFIXED((sq_INT_BITS-1) downto -sq_FRAC_BITS);
    signal tempcos697, tempcos941, tempcos1477 : SFIXED((sq_INT_BITS-1) downto -sq_FRAC_BITS);
...
begin
    ...
    begin
        if reset = '1' then
            out_valid <= '0';
            state <= IDLE;
            tempsin697 <= (others => '0'); tempsin941 <= (others => '0'); tempsin1477<= (others => '0');
            tempcos697 <= (others => '0'); tempcos941 <= (others => '0'); tempcos1477<= (others => '0');
            power_697 <= (others => '0'); power_941 <= (others => '0'); power_1477 <= (others => '0');
            ...
            case state is
                when IDLE =>
                    ...
                    out_valid <= '0';
                    if in_valid = '1' then state <= COMPUTE;
                    else state <= IDLE;
                    end if;
                when COMPUTE =>
                    ...
                    state <= STORE;
                when STORE =>
                    out_697  <= power_697; out_941  <= power_941; out_1477 <= power_1477;
                    out_valid <= '1';
                    if out_ready = '1' then state <= IDLE; end if;
                when others =>
                    state <= IDLE;
            end case;
        end if;
