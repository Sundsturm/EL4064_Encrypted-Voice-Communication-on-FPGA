...
% Ini versi simulasi filter Goertzel yang menggunakan koefisien
% dari textbook DSP oleh Jiang dan Tan (2018) tetapi hanya bagian
% awalnya (ternyata belum lengkap).

% Terdapat 3 input yang dapat dicoba per run di inputX1, inputX2, inputX3.
% Untuk versi program yang ini, diberikan input 697 Hz dengan variasi:
% inputX1: 2500 data noise + (Fs*durasi) data tone 697 Hz + 2500 data noise
% inputX2: (Fs*durasi) data tone 697 Hz + 5000 data noise 
% inputX3: 5000 data noise + (Fs*durasi) data tone 697 Hz

% Parameter frekuensi filter Goertze ke-1, 2, dan 3 dilakukan dengan
% mengatur nilai goertzelFreq1, goertzelFreq2, dan goertzelFreq3.
...
% CONFIGURE INPUT
inputX1 = [(4*rand(1, 2500))-2, tones(1,:), 4*(rand(1, 2500))-2];
inputName1 = '697 Hz v1';

inputX2 = [tones(1,:), 4*(rand(1, 5000))-2];
inputName2 = '697 Hz v2';

inputX3 = [(4*rand(1, 5000))-2, tones(1,:)];
...
function Y = goertzel3(X, f, fs)
    ...
    for m = 3:length(Y)
        Y(m) = (2 * cos(2*pi*k) * Y(m-2)) - Y(m-1) + X(m);
    end
end