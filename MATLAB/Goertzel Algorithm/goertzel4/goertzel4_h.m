...
% goertzel4_f but with single precision
...
k_indices = single(round(dtmf_freqs * N_total / Fs));
...
function [yn, Ak, filter_output] = goertzel4(x, k)
    ...
    for n = 1:N+1
        vk(n+2) = single(2*cos(2*pi*k/N)*vk(n+1)) - single(vk(n)) + single(x(n));
        if n <= N
            filter_output(n) = single(vk(n+2));
        end
    end

    yn = single(vk(N+3) - exp(-2*pi*1i*k/N)*vk(N+2));
