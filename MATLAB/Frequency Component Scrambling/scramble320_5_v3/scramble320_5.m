...
doc_subtitle = sprintf('320 samples instead of 960; samples 1-50 and 271-320');
...
for ex = 1:size(experiments, 1) 
    ...
    for m = 1:counter
        ...
        temp1(1:80) = [Z(m, a1:a2) Z(m, b1:b2) Z(m, c1:c2) Z(m, d1:d2) Z(m, e1:e2) Z(m, f1:f2) Z(m, g1:g2) Z(m, h1:h2) Z(m, i1:i2) Z(m, j1:j2) Z(m, k1:k2) Z(m, l1:l2) Z(m, m1:m2) Z(m, n1:n2) Z(m, o1:o2) Z(m, p1:p2)];
        temp2(1:80) = [Zi(m, a1:a2) Zi(m, b1:b2) Zi(m, c1:c2) Zi(m, d1:d2) Zi(m, e1:e2) Zi(m, f1:f2) Zi(m, g1:g2) Zi(m, h1:h2) Zi(m, i1:i2) Zi(m, j1:j2) Zi(m, k1:k2) Zi(m, l1:l2) Zi(m, m1:m2) Zi(m, n1:n2) Zi(m, o1:o2) Zi(m, p1:p2)];

        a1 = experiments(ex, 1)*5+241;
        a2 = (experiments(ex, 1)+1)*5+240;
        b1 = experiments(ex, 2)*5+241;
        b2 = (experiments(ex, 2)+1)*5+240;
        c1 = experiments(ex, 3)*5+241;
        c2 = (experiments(ex, 3)+1)*5+240;
        d1 = experiments(ex, 4)*5+241;
        d2 = (experiments(ex, 4)+1)*5+240;
        e1 = experiments(ex, 5)*5+241;
        e2 = (experiments(ex, 5)+1)*5+240;
        f1 = experiments(ex, 6)*5+241;
        f2 = (experiments(ex, 6)+1)*5+240;
        g1 = experiments(ex, 7)*5+241;
        g2 = (experiments(ex, 7)+1)*5+240;
        h1 = experiments(ex, 8)*5+241;
        h2 = (experiments(ex, 8)+1)*5+240;
        i1 = experiments(ex, 9)*5+241;
        i2 = (experiments(ex, 9)+1)*5+240;
        j1 = experiments(ex, 10)*5+241;
        j2 = (experiments(ex, 10)+1)*5+240;
        k1 = experiments(ex, 11)*5+241;
        k2 = (experiments(ex, 11)+1)*5+240;
        l1 = experiments(ex, 12)*5+241;
        l2 = (experiments(ex, 12)+1)*5+240;
        m1 = experiments(ex, 13)*5+241;
        m2 = (experiments(ex, 13)+1)*5+240;
        n1 = experiments(ex, 14)*5+241;
        n2 =  (experiments(ex, 14)+1)*5+240;
        o1 = experiments(ex, 15)*5+241;
        o2 = (experiments(ex, 15)+1)*5+240;
        p1 = experiments(ex, 16)*5+241;
        p2 = (experiments(ex, 16)+1)*5+240;

        temp1(241:320) = [Z(m, a1:a2) Z(m, b1:b2) Z(m, c1:c2) Z(m, d1:d2) Z(m, e1:e2) Z(m, f1:f2) Z(m, g1:g2) Z(m, h1:h2) Z(m, i1:i2) Z(m, j1:j2) Z(m, k1:k2) Z(m, l1:l2) Z(m, m1:m2) Z(m, n1:n2) Z(m, o1:o2) Z(m, p1:p2)];
        temp2(241:320) = [Zi(m, a1:a2) Zi(m, b1:b2) Zi(m, c1:c2) Zi(m, d1:d2) Zi(m, e1:e2) Zi(m, f1:f2) Zi(m, g1:g2) Zi(m, h1:h2) Zi(m, i1:i2) Zi(m, j1:j2) Zi(m, k1:k2) Zi(m, l1:l2) Zi(m, m1:m2) Zi(m, n1:n2) Zi(m, o1:o2) Zi(m, p1:p2)];
...
for ex = 1:size(experiments, 1) 
    ...
    for m = 1:counter
        ...
        temp1(1:80) = [X(m, a1:a2) X(m, b1:b2) X(m, c1:c2) X(m, d1:d2) X(m, e1:e2) X(m, f1:f2) X(m, g1:g2) X(m, h1:h2) X(m, i1:i2) X(m, j1:j2) X(m, k1:k2) X(m, l1:l2) X(m, m1:m2) X(m, n1:n2) X(m, o1:o2) X(m, p1:p2)];
        temp2(1:80) = [Xi(m, a1:a2) Xi(m, b1:b2) Xi(m, c1:c2) Xi(m, d1:d2) Xi(m, e1:e2) Xi(m, f1:f2) Xi(m, g1:g2) Xi(m, h1:h2) Xi(m, i1:i2) Xi(m, j1:j2) Xi(m, k1:k2) Xi(m, l1:l2) Xi(m, m1:m2) Xi(m, n1:n2) Xi(m, o1:o2) Xi(m, p1:p2)];
        ...
        temp1(241:320) = [X(m, a1:a2) X(m, b1:b2) X(m, c1:c2) X(m, d1:d2) X(m, e1:e2) X(m, f1:f2) X(m, g1:g2) X(m, h1:h2) X(m, i1:i2) X(m, j1:j2) X(m, k1:k2) X(m, l1:l2) X(m, m1:m2) X(m, n1:n2) X(m, o1:o2) X(m, p1:p2)];
        temp2(241:320) = [Xi(m, a1:a2) Xi(m, b1:b2) Xi(m, c1:c2) Xi(m, d1:d2) Xi(m, e1:e2) Xi(m, f1:f2) Xi(m, g1:g2) Xi(m, h1:h2) Xi(m, i1:i2) Xi(m, j1:j2) Xi(m, k1:k2) Xi(m, l1:l2) Xi(m, m1:m2) Xi(m, n1:n2) Xi(m, o1:o2) Xi(m, p1:p2)];
